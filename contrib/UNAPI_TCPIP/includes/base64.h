...
void Base64Init(byte charsPerLine) __sdcccall(0);
...
uint Base64EncodeChunk(byte* source, byte* destination, uint length, byte final) __sdcccall(0);
...
uint Base64DecodeChunk(byte* source, byte* destination, uint length, byte final, byte* error) __sdcccall(0);
